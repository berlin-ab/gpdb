...
static bool is_tablespace_empty(Oid tablespace_oid);
static void ensure_tablespace_directory_is_empty(Oid tablespaceoid);
...
bool
is_tablespace_empty(Oid tablespace_oid)
{
...
static void 
ensure_tablespace_directory_is_empty(Oid tablespaceoid) {
	if (is_tablespace_empty(tablespaceoid))
		return;
	...
	ereport(ERROR, (errmsg("cannot delete tablespace, it is not empty.")));
}
...
DropTableSpace(DropTableSpaceStmt *stmt)
{
	...
	ensure_tablespace_directory_is_empty(tablespaceoid);
...
tblspc_redo(XLogRecPtr beginLoc, XLogRecPtr lsn, XLogRecord *record)
{
	...
	{
		// TODO: do we need to do ResolveRecoveryConflictWithTablespaces
		// TODO: What about log messages that were here before?
	}
	else
